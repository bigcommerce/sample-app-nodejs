...
const InternalOrderPage = (order: ShippingAndProductsInfo) => {
  ...
  const items = shipping_addresses.map((address) => {
    const addressProducts = products.filter(
      ({ order_address_id }) => order_address_id === address.id
    )
  ...
  const renderOrderProductsQuantities = (addressProducts: OrderProduct[]) => (
    <>
      {addressProducts.map((product) => (
        <Text key={product.id}>{product.quantity}</Text>
      ))}
    </>
  )
...
const OrderPage = () => {
  ...
  const { isLoading, order, error } = useShippingAndProductsInfo(
    parseInt(`${orderId}`, 10)
  )
